...
df_bt = apply_ml_filter(
    df_bt,
    model_name="xgb_ttp",
    proba_threshold=0.85
)
df_bt.loc[~df_bt["ML_ALLOW_ENTRY"], "EntrySignal"] = 0
req = [
...
for i in range(len(df_bt)):
    ...
    if _pos == 0 and df_bt.at[i, "ExitSignal"] == 0:
        ...
        if s != 0 and float(df_bt.at[i, "ML_PROBA_GOOD"]) < 0.85:
            s = 0
        if s != 0:
            _pos = s
...
for i in range(len(df_bt)):
    ...
    if (not in_trade) and (pending is not None) and (i == pending["idx"] + 1):
        ...
        if not bool(df_bt.at[pending["idx"], "ML_ALLOW_ENTRY"]):
            pending = None
            continue
        if trig:
    ...
    if in_trade and wait_addon and (not addon_done):
        ...
        if trig_add and df_bt.at[i, "ML_PROBA_GOOD"] > 0.6:
            fill_add = px_addon(i)
    ...
    if (not in_trade) and (pending is None) and ex == 0:
        ...
        if s != 0:
            print(
                df_bt.at[i, "DateTime"],
                "signal:", s,
                "ml_allow:", df_bt.at[i, "ML_ALLOW_ENTRY"],
                "proba:", df_bt.at[i, "ML_PROBA_GOOD"]
            )
        if s != 0 and float(df_bt.at[i, "ML_PROBA_GOOD"]) < 0.85:
            s = 0
